...
class GenerateDocumentNumber
{
    ...
    public static function generateNumber($documentType, $companyCode, $departmentCode, $signBy, $lastNumber = '')
    {
        ...
        $generateNumber = '001';
        if ($lastNumber != '') {
            // Cek apakah tahun telah berubah sejak nomor terakhir dibuat
            $lastYearDocument = Document::orderBy('created_at', 'desc')->value('created_at');
            $lastYearGenerate = GenerateNumber::orderBy('created_at', 'desc')->value('created_at');

            if ($lastYearDocument && $lastYearGenerate && date('y', strtotime($lastYearDocument)) == $yearLastTwoDigits && date('y', strtotime($lastYearGenerate)) == $yearLastTwoDigits) {
                // Jika tahun masih sama, lanjutkan penomoran
