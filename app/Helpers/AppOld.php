...
{
    $contract = RequestDocument::where('status', '<', 7)->where('type', 'Contract')->where('is_cancel', 0)->count();
...
{
    $contract = RequestDocument::where('status', '<', 4)->where('type', 'License')->where('is_cancel', 0)->count();
...
{
    $contract = RequestExisting::where('status', '<', 5)->where('is_deleted', false)->where('is_cancel', false)->count();
...
{
    $request = RequestDocumentQR::where('status_action', 'New')->count();
...
function countRequest()
{
    ...
    $existing = RequestExisting::where('status', '<', 5)->where('is_deleted', false)->where('is_cancel', false)->count();
...
function getTemplateByUser()
{
    try {
        ...
        return Template::where('company_name', Auth::user()->company_name)->get();
    } catch (Exception $e) {
        return redirect()->back()->with('error', 'Error Request, Exception Error ');
    }
}
