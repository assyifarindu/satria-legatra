...
use App\Models\Table\TspRequestDocumentFeedbackFileDownloads;
...
class DownloadFileController extends Controller
{
     ...
     * @return \Illuminate\Http\Response
     */
    public function downloadRequestDocumentFile($fileId)
    {
        try {
            $user = Auth::user();
    ...
    /**
     * Download feedback file with timestamp footer
     * @param int $fileId
     * @return \Illuminate\Http\Response
     */
    public function downloadFeedbackFile($fileId)
    {
        try {
            $user = Auth::user();
            $file = TspRequestDocumentFeedbackFile::findOrFail($fileId);
            ...
            // 3. Simpan data download user ke database
            TspRequestDocumentFeedbackFileDownloads::create([
                'request_document_feedback_file_id' => $file->id,
                'download_by' => $user->id,
                'download_at' => now(),
                'created_by' => $user->id,
                'created_at' => now()
