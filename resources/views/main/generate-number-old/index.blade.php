                                ...
                                <tbody>
                                    @foreach ($documentNumbers as $item)
                                        <tr>
                                            ...
                                            <td class="text-success">{{ $item->document_number }}</td>
                                            <td>{{ $item->user }}</td>
                                            ...
                                            <td class="align-middle">
                                                <a href="{{ route('generate-number.edit', Hashids::encode($item->id)) }}"
                                                ...
                                                <form class="d-inline" action="{{ route('generate-number.destroy', $item) }}" method="post"
                                                    id="deleteForm{{ $item->id }}">
                                                    @method('delete')
                                                    @csrf
                                                </form>
                                                <a class="btn btn-light btn-xs d-inline waves-effect waves-light btn_view" title="Hapus" tabindex="0" data-plugin="tippy"
                                                    data-tippy-placement="top"
                                                        onclick="confirmDelete({{ $item->id }})"><i class="fas fa-trash"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
