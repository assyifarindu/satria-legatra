                    ...
                    <div class="page-title-box">
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                ...
                                <li class="breadcrumb-item active">Request Extend Document</li>
                            </ol>
                        </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <div class="float-end">
                                <a href="{{ route('request-extend.create') }}" class="btn btn-sm btn-primary"><i
                                        class="fas fa-plus"></i> Create Request</a>
                            </div>
                            <h4 class="header-title">Data Request Extend Document</h4>
                            <br><br>
                                        ...
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                @if ($item->type == 'License')
                                                    <a
                                                        href="{{ route('request-extend.show-license', Hashids::encode($item->id)) }}">{{ Str::limit($item->title, 50, '...') }}</a>
                                                @elseif ($item->type == 'HAKI')
                                                    <a
                                                        href="{{ route('request-extend.show-haki', Hashids::encode($item->id)) }}">{{ Str::limit($item->title, 50, '...') }}</a>
                                                @else
                                                    <a
                                                        href="{{ route('request-extend.show', Hashids::encode($item->id)) }}">{{ Str::limit($item->title, 50, '...') }}</a>
                                                @endif
                                            </td>
                                            <td class="align-middle">
                                            ...
                                            <td class="align-middle">
                                                @if ($item->status == 0 && $item->is_cancel == 0)
                                                    <a href="{{ route('request-extend.edit', Hashids::encode($item->id)) }}"
                                                ...
                                                @endif
                                                @if ($item->type == 'License')
                                                    <a href="{{ route('request-extend.show-license', Hashids::encode($item->id)) }}"
                                                        class="btn btn-light btn-xs d-inline waves-effect waves-light btn_view"
                                                        title="View Detail" tabindex="0" data-plugin="tippy"
                                                        data-tippy-placement="top"><i class="fas fa-eye"></i></a>
                                                @elseif ($item->type == 'HAKI')
                                                    <a href="{{ route('request-extend.show-haki', Hashids::encode($item->id)) }}"
                                                        class="btn btn-light btn-xs d-inline waves-effect waves-light btn_view"
                                                        title="View Detail" tabindex="0" data-plugin="tippy"
                                                        data-tippy-placement="top"><i class="fas fa-eye"></i></a>
                                                @else
                                                    <a href="{{ route('request-extend.show', Hashids::encode($item->id)) }}"
                                                        class="btn btn-light btn-xs d-inline waves-effect waves-light btn_view"
                                                        title="View Detail" tabindex="0" data-plugin="tippy"
                                                        data-tippy-placement="top"><i class="fas fa-eye"></i></a>
                                                @endif
    ...
    <div id="cancel-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered modal-md">
            <div class="modal-content">
                ...
                <form action="{{ route('request-extend.cancel') }}" method="post">
                    @csrf
                    <div class="modal-body">
    ...
    <script>
        $(document).ready(function() {
            $('.history_process').click(function() {
                ...
                $.ajax({
                    url: '{{ url('request-extend/get-data') }}/' + doc_id,
                    type: 'get',
                    data: {
                        doc_id: doc_id
                    },
