            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <div class="float-end">
                                <a href="{{ route('request-existing.create') }}" class="btn btn-sm btn-primary"><i
                                        ...
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                <ul>
                                                    @foreach ($item->RequestExistingDocument as $rd)
                                                        <li><a
                                                                href="{{ route('request-existing.show', Hashids::encode($item->id)) }}">{{ Str::limit($rd->title, 50, '...') }}</a>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                            ...
                                            <td class="align-middle">
                                                @if ($item->status == 1 && $item->is_cancel == 0)
                                                    <a href="{{ route('request-existing.edit', Hashids::encode($item->id)) }}"
                                                ...
                                                @endif
                                                <a href="{{ route('request-existing.show', Hashids::encode($item->id)) }}"
                                                ...
                                                @if ($item->status == 1 && $item->is_cancel == 0)
                                                    <form
                                                        action="{{ route('request-existing.destroy', Hashids::encode($item->id)) }}"
    ...
    <script>
        $(document).ready(function() {
            $('.history_process').click(function() {
                ...
                $.ajax({
                    url: '{{ url('request-existing/get-data') }}/' + doc_id,
                    type: 'get',
                    data: {
                        doc_id: doc_id
                    },
